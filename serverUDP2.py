...
while 1:
    ...
    ##Try to get the file size on disk
    bSize=os.statvfs(fileToClient).f_bsize
    print 'The real size of the file on disk is:  ', bSize
    
    ## OK downloaded file form client
    okMessage,clientAddress  = serverSocket.recvfrom(2048)
    print 'From client: ' , okMessage
    
    ## Sending the real size on disk
    print 'Sending the size'
    serverSocket.sendto(str(bSize), clientAddress)
